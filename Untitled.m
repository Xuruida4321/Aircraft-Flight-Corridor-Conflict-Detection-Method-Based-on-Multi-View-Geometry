...
% 检查读取的数据大小，确保其符合预期（例如，100行x16列）
disp(size(inputParams));
results = zeros(30, 2); % 假设每组输入会产生一个结果和一个时间值
...
% 结束计时并记录结果
    elapsedTime = toc;
%     results(i, 1) = isIntersecting; % 假设这是您的计算结果
%     results(i, 2) = elapsedTime;
    results(idx, 1) = isIntersecting; % 这是您的计算结果
    results(idx, 2) = elapsedTime; % 这是计算时间
    fprintf('当前结果：%d, 时间：%.4f\n', isIntersecting, elapsedTime);
